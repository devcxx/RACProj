...
qint32 QMqttClient::publish(const QString &topic, const QString &message, quint8 qos, bool retain)
{
    return publish(QMqttTopicName(topic), message.toUtf8(), qos, retain);
}
...
void QMqttClient::setPort(int port)
{
    Q_D(QMqttClient);
    if (d->m_port == port)
        return;
